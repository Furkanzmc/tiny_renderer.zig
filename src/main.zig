...
pub fn main() !void {
...

    {
        ...
        const model_path = try std.fmt.allocPrint(allocator, "{s}/{s}", .{ current_dir, "test_assets/cube.obj" });
        try model.read(model_path);

        renderer.render_model(&model, &image, WHITE);

        try image.writeFilepath(try std.fmt.allocPrint(allocator, "{s}/{s}", .{ current_dir, "zig-out/cube.tga" }), false);
    }
